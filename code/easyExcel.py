...
class easyExcel:
    ...
    def __init__(self, filename=None):  # 打开文件或者新建文件（如果不存在的话）
        ...
        self.xlApp.Visible = 0  # 后台运行，不显示，不警告
    ...
    def setCell(self, sheet, row, col, value):  # 设置单元格的数据
        ...
        sht = self.xlBook.Worksheets(sheet)
        value2string = str(value)
        if value2string.isnumeric():
            value2string = "'" + value2string
        sht.Cells(row, col).Value = value2string
    ...
    def getRowNumber(self,ColumnRange): #得到行号
        currentRow = 1
        while True:
            i = 1
            Flag = False
            while i <= ColumnRange:
                currentCell = str(self.getCell('sheet1',currentRow,i))
                if currentCell != 'None':
                    Flag = True
                    break
                i += 1
...
if __name__ == "__main__":
    ...
    while (row < 10):
        while (col < 5):
            xls.setCell('sheet1', row, col, 'chen')
            col += 1
        row += 1
        col = 1
    '''
    print(xls.getRowNumber(4))
    xls.save('E:\\workProjects\\NetLab530\\depression\\text3.xls')
    xls.close()